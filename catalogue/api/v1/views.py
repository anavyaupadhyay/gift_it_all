...
from gift_it_all.utils import parse_basket_from_hyperlink
    ...
    def post(self, request, format=None, *args, **kwargs):
        ...
        if c_ser.is_valid():
            ...
            if user_transaction_id:
                TransationID=settings.PROD_TRANSATION_ID
                # STAG_TRANSATION_ID="INSTASTAG"
                # PROD_TRANSATION_ID="INSTAPRODPAY"
                user_transaction_id=user_transaction_id.replace(TransationID,'')
            ...
            if hasattr(settings, 'OSCAR_INITIAL_ORDER_STATUS'):
                ...
                order.payment_mode = request.data.get('payment_mode', None)
                order.save()
            basket.freeze()
...
class AddVoucherView(basket.AddVoucherView):
    ...
    def post(self, request, *args, **kwargs):  # pylint: disable=redefined-builtin
        ...
        if v_ser.is_valid():
            ...
            # al_lines=basket.lines.values('price_excl_tax')
            # total=al_lines.aggregate(total_sum=Sum('price_excl_tax'))['total_sum']
            total=0
            for line in basket.lines.all():
                qty=line.quantity
                price=line.price_excl_tax *qty
                total+=price
            
            if not total>=voucher.discount_up_to:
                return Response(
                    {
                        "reason": _(
